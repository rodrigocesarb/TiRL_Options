...
public class QLTutorial extends MDPSolver implements LearningAgent, QProvider {
	...
	public Episode runLearningEpisode(Environment env, int maxSteps) {
		...
		while(!env.isInTerminalState() && (steps < maxSteps || maxSteps == -1)){
			...
			curState = eo.op;
			steps++;
			System.out.println("Episodio "+epi);
			System.out.println("STEPS "+steps);
			recompensaDescontada = eo.r - (0.9 * steps);

			System.out.println("Recompensa Descontada "+recompensaDescontada);
			epi++;
		}
