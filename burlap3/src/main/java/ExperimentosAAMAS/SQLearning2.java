 ...
 *
 */
public class SQLearning2 extends MDPSolver implements QProvider, LearningAgent, Planner{
	...
	public GreedyQPolicy2												gqp;

	public BasicBehavior												bb;
	 ...
	 */
	//original
	public SQLearning2(SADomain domain, double gamma, HashableStateFactory hashingFactory,
	...
	//	//meu
	//	public QLearning2(SADomain domain, double gamma, HashableStateFactory hashingFactory,
	//			double qInit, double learningRate) {
	//		this.QLInit(domain, gamma, hashingFactory, new ConstantValueFunction(qInit), learningRate, new EpsilonGreedy(this, 0.1), Integer.MAX_VALUE);
	//	}
	...
	//meu
	public SQLearning2(SADomain domain, double gamma, HashableStateFactory hashingFactory,
			double qInit, GreedyQPolicy2 gqp, double learningRate) {
		this.QLInit(domain, gamma, hashingFactory, new ConstantValueFunction(qInit), learningRate, gqp, Integer.MAX_VALUE);
	}
	...
	//meu
	public SQLearning2(SADomain domain, double gamma, HashableStateFactory hashingFactory,
			double qInit, double learningRate, GreedyQPolicy gqp, int maxEpisodeSize) {
		this.QLInit(domain, gamma, hashingFactory, new ConstantValueFunction(qInit), learningRate, gqp, maxEpisodeSize);
	}
	...
	//meu
	public SQLearning2(SADomain domain, double gamma, HashableStateFactory hashingFactory,
			QFunction qInit, double learningRate, GreedyQPolicy2 gqp, int maxEpisodeSize) {
		this.QLInit(domain, gamma, hashingFactory, qInit, learningRate, learningPolicy, maxEpisodeSize);
	}
	...
	//meu
	protected void QLInit(SADomain domain, double gamma, HashableStateFactory hashingFactory,
			QFunction qInitFunction, double learningRate, Policy learningPolicy, GreedyQPolicy2 gqp, int maxEpisodeSize){
	...
	public void setLearningGreedyPolicy(GreedyQPolicy2 gqp){
		this.gqp = gqp;
	}
	...
	//meu
	public Hashtable<HashableState, Action> runLearningEpisodeWithHash(Environment env, int maxSteps) {
		State initialState = env.currentObservation();
		...
		Hashtable<HashableState, Action> hs = new Hashtable<HashableState, Action>();
		...
		while(!env.isInTerminalState() && (eStepCounter < maxSteps || maxSteps == -1)){
			...
			//converte hashing em estado
			Action action = learningPolicy.action(curState.s());
			
//			Action action = gqp.action(curState);
			...
			if(!(action instanceof Option) || !this.shouldDecomposeOptions){
				hs.put(nextState, action);
			}
//			else{
//				hs.appendAndMergeEpisodeAnalysis(((EnvironmentOptionOutcome)eo).episode);
//			}
